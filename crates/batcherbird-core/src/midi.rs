...
impl MidiManager {
    ...
    pub fn connect_output(&mut self, device_index: usize) -> Result<MidiOutputConnection> {
        let midi_out = match self.output.take() {
            Some(output) => output,
            None => MidiOutput::new("batcherbird-output").map_err(|e| {
                BatcherbirdError::Session(format!("Failed to create MIDI output: {:?}", e))
            })?,
        };
        ...
        let conn_out = midi_out
            .connect(port, &format!("batcherbird-out-{}", device_name))
            .map_err(|e| {
                BatcherbirdError::Session(format!("Failed to connect to MIDI output: {:?}", e))
            })?;
    ...
    pub fn connect_input(&mut self, device_index: usize) -> Result<MidiInputConnection<()>> {
        ...
        let conn_in = midi_in
            ...
            .map_err(|e| {
                BatcherbirdError::Session(format!("Failed to connect to MIDI input: {:?}", e))
            })?;
    ...
    pub fn send_channel_panic(conn: &mut MidiOutputConnection, channel: u8) -> Result<()> {
        ...
        let all_notes_off = [0xB0 | channel, 123, 0];
        conn.send(&all_notes_off).map_err(|e| {
            BatcherbirdError::Session(format!("Failed to send All Notes Off: {:?}", e))
        })?;

        // Send Reset All Controllers CC
        let reset_controllers = [0xB0 | channel, 121, 0];
        conn.send(&reset_controllers).map_err(|e| {
            BatcherbirdError::Session(format!("Failed to send Reset Controllers: {:?}", e))
        })?;
