...
impl FftCorrelator {
    ...
    pub fn fft_autocorrelation(&mut self, signal: &[f32]) -> Result<Vec<f32>> {
        ...
        let (mut fft_buffer, mut spectrum) = if let Some((fft_buf, spec)) = self.cache.remove(&fft_size) {
            (fft_buf, spec)
        } else {
        ...
        let mut autocorr: Vec<f32> = spectrum.iter()
            .take(n)
            .map(|c| c.re / n as f32)
            .collect();
...
impl SpectralCoherence {
    ...
    pub fn calculate_coherence(&mut self, start_segment: &[f32], end_segment: &[f32]) -> Result<f32> {
    ...
    fn compute_spectrum(&self, fft: &std::sync::Arc<dyn rustfft::Fft<f32>>, signal: &[f32]) -> Result<Vec<Complex<f32>>> {
        let mut buffer = vec![Complex::new(0.0, 0.0); self.window_size];
        
        // Copy signal and apply Hann window
        for (i, &sample) in signal.iter().enumerate() {
            let window_val = 0.5 * (1.0 - (2.0 * std::f32::consts::PI * i as f32 / self.window_size as f32).cos());
...
impl MultiscaleAnalysis {
    ...
    pub fn analyze_at_scales(&self, correlator: &mut FftCorrelator, signal: &[f32]) -> Result<Vec<(usize, f32)>> {
...
impl AdvancedLoopDetector {
    ...
    pub fn detect_loops(&mut self, audio_data: &[f32], sample_rate: u32) -> Result<AdvancedLoopResult> {
        ...
        sorted_candidates.sort_by(|a, b| b.quality.overall_score.partial_cmp(&a.quality.overall_score).unwrap_or(std::cmp::Ordering::Equal));
        
        // Take top candidates and validate
        sorted_candidates.truncate(self.config.candidate_count);
        
        let best_loop = sorted_candidates.first().cloned().filter(|c| c.quality.overall_score >= self.config.correlation_threshold);
        ...
        Ok(AdvancedLoopResult {
            ...
            algorithm_used: if self.config.multi_scale { "MultiScale-FFT" } else { "FFT-Autocorr" }.to_string(),
        })
    }
    
    fn detect_multiscale_loops(&mut self, audio_data: &[f32], sample_rate: u32) -> Result<Vec<LoopCandidate>> {
        let multiscale = MultiscaleAnalysis::new(sample_rate, self.config.min_loop_length, self.config.max_loop_length);
        let scale_results = multiscale.analyze_at_scales(&mut self.correlator, audio_data)?;
        
        let mut candidates = Vec::new();
        
        for (scale, correlation) in scale_results.into_iter().take(self.config.candidate_count * 2) {
            if correlation >= self.config.correlation_threshold {
                let candidate = self.create_loop_candidate(audio_data, sample_rate, scale, correlation)?;
    ...
    fn detect_correlation_loops(&mut self, audio_data: &[f32], sample_rate: u32) -> Result<Vec<LoopCandidate>> {
        ...
        for i in min_samples..autocorr.len().min(max_samples) {
            if autocorr[i] >= self.config.correlation_threshold {
                ...
                if is_peak {
                    let candidate = self.create_loop_candidate(audio_data, sample_rate, i, autocorr[i])?;
                    if let Some(c) = candidate {
                        candidates.push(c);
                    }
                }
    ...
    fn create_loop_candidate(&mut self, audio_data: &[f32], sample_rate: u32, loop_length: usize, correlation: f32) -> Result<Option<LoopCandidate>> {
        ...
        let start_segment = &audio_data[start_sample..start_sample + analysis_length.min(loop_length)];
        let end_segment = &audio_data[end_sample - analysis_length.min(loop_length)..end_sample];
        
        let spectral_coherence = self.spectral_analyzer.calculate_coherence(start_segment, end_segment)?;
        ...
        let overall_score = (correlation * 0.4) + (spectral_coherence * 0.3) + (phase_alignment * 0.3);
...
mod tests {
    ...
    fn test_multiscale_analysis() {
        ...
        let results = multiscale.analyze_at_scales(&mut correlator, &signal).unwrap();
