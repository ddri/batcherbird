...
impl BatchProcessor {
        ...
        let thread_pool = if let Some(max_threads) = config.max_threads {
            Some(
                rayon::ThreadPoolBuilder::new()
                    ...
                    .build()
                    .map_err(|e| {
                        BatcherbirdError::Audio(format!("Failed to create thread pool: {}", e))
                    })?,
            )
        } else {
            ...
            let chunk_results: Vec<_> = chunk
                ...
                .map(|sample| {
                    if progress.is_cancelled() {
                        return (
                            sample.id.clone(),
                            Err(BatcherbirdError::Audio("Operation cancelled".to_string())),
                        );
                    }
        ...
        T: Send,
    {
        ...
        if elapsed > timeout {
            ...
            return Ok((
                sample.id.clone(),
                Err(BatcherbirdError::Audio(format!(
...
mod tests {
    ...
    fn test_sample_data_memory_calculation() {
        let sample = SampleData::new("test".to_string(), vec![1.0, 2.0, 3.0, 4.0], 44100);
    ...
    fn test_parallel_processing() {
        ...
        // Create test samples
        let samples = (0..10)
            .map(|i| SampleData::new(format!("sample_{}", i), vec![i as f32; 100], 44100))
            .collect();

        // Simple operation: sum all audio samples
        let operation = |sample: &SampleData| -> Result<f32> { Ok(sample.audio_data.iter().sum()) };
    ...
    fn test_error_recovery() {
        ...
        let samples = (0..5)
            .map(|i| SampleData::new(format!("sample_{}", i), vec![i as f32; 10], 44100))
            .collect();
    ...
    fn test_streaming_processor() {
        ...
        let samples = (0..5)
            .map(|i| SampleData::new(format!("stream_sample_{}", i), vec![i as f32; 10], 44100));

        let operation = |sample: &SampleData| -> Result<usize> { Ok(sample.audio_data.len()) };

        let results: Vec<_> = processor
            .process_stream(samples, operation)
            .unwrap()
            .collect();
