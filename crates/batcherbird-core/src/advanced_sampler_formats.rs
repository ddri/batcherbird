use crate::{Result, BatcherbirdError, ProfessionalMetadata};
...
impl VelocityLayerEngine {
    ...
    pub fn generate_velocity_layers(&self, samples: &[SampleInfo]) -> Result<Vec<VelocityLayer>> {
        ...
        println!("✅ Generated {} velocity layers with {:.1}% crossfade overlap", 
            layers.len(), self.crossfade_overlap * 100.0);
        
        Ok(layers)
    }
    
    fn calculate_gain_offset(&self, sample_info: &SampleInfo, layer_index: usize, total_layers: usize) -> f32 {
        ...
        let sample_gain = sample_info.metadata.get("gain_offset")
...
impl RoundRobinProcessor {
    ...
    pub fn create_round_robin_groups(
        ...
        println!("✅ Created {} round-robin groups with up to {} samples each", 
            groups.len(), self.max_samples_per_group);
...
impl AdvancedSamplerExporter {
    ...
    pub fn export_decent_sampler(
        ...
        println!("✅ Exported DecentSampler preset: {}", output_path.display());
        Ok(())
    }
    ...
    pub fn export_sfz(&self, instrument: &AdvancedInstrument, output_path: &Path) -> Result<()> {
        ...
        sfz_content.push_str(&format!("// Generated by BatcherBird\n\n"));
        
        // Control section
        sfz_content.push_str("<control>\n");
        sfz_content.push_str(&format!("default_path={}\n", 
        ...
        if let Some(ref envelope) = instrument.global_settings.global_envelope {
            ...
            sfz_content.push_str(&format!("ampeg_sustain={:.1}\n", envelope.sustain_level * 100.0));
        ...
        for (articulation_name, articulation) in &instrument.articulations {
            ...
            match &articulation.trigger {
                ArticulationTrigger::MidiCC { controller, value_range } => {
                    sfz_content.push_str(&format!("locc{}={} hicc{}={}\n", 
                        controller, value_range.0, controller, value_range.1));
                }
                ArticulationTrigger::Keyswitch { note, .. } => {
                    sfz_content.push_str(&format!("sw_last={}\n", note));
                }
                ArticulationTrigger::VelocityRange { min_velocity, max_velocity } => {
                    sfz_content.push_str(&format!("lovel={} hivel={}\n", min_velocity, max_velocity));
                }
            ...
            for layer in &articulation.velocity_layers {
                ...
                if let Some(ref loop_points) = layer.loop_points {
                    sfz_content.push_str(&format!("loop_mode=loop_continuous\n"));
        ...
        println!("✅ Exported SFZ instrument: {}", output_path.display());
        Ok(())
    }
    
    fn write_decent_sampler_xml(&self, preset: &DecentSamplerPreset, output_path: &Path) -> Result<()> {
        // Simplified XML generation - in production would use proper XML library
        let mut xml = String::new();
        xml.push_str("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
        xml.push_str("<DecentSampler>\n");
        xml.push_str(&format!("  <groups>\n"));
        ...
        std::fs::write(output_path, xml)
            .map_err(|e| BatcherbirdError::Audio(format!("Failed to write DecentSampler XML: {}", e)))?;
...
mod tests {
    ...
    fn test_round_robin_groups() {
        ...
        let groups = processor.create_round_robin_groups(&samples, "test").unwrap();
    ...
    fn test_advanced_instrument_creation() {
        ...
        instrument.articulations.insert("sustain".to_string(), articulation);
    ...
    fn test_sample_info() {
        ...
        sample.metadata.insert("gain_offset".to_string(), "-2.5".to_string());
        sample.peak_level_db = Some(-6.0);
        sample.rms_level_db = Some(-18.0);
        
        assert_eq!(sample.path, PathBuf::from("test.wav"));
        assert_eq!(sample.metadata.get("gain_offset"), Some(&"-2.5".to_string()));
