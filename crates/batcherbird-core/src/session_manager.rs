...
impl SessionManager {
    ...
    pub fn initialize_session(&mut self, config: SessionConfig) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !validation_report.is_valid {
            ...
            return Err(format!("Session validation failed: {} errors", validation_report.errors.len()).into());
        }
    ...
    pub fn validate_recording_state(&self) -> Result<(), Box<dyn std::error::Error>> {
        match self.session_state {
            ...
            SessionState::Uninitialized => Err("Session not initialized - please create a new session".into()),
    ...
    fn test_audio_input_device(&self, device_name: &str, config: &AudioSessionConfig) -> TestResult {
        // TODO: Implement actual audio input test
        // For now, return success if device is in available list
        if self.config_validator.available_audio_inputs.contains(&device_name.to_string()) {
            TestResult {
                success: true,
                message: format!("Audio input '{}' is available", device_name),
                details: Some(format!("Sample rate: {}Hz, Bit depth: {}bit", config.sample_rate, config.bit_depth)),
    ...
    fn test_audio_output_device(&self, device_name: &str, config: &AudioSessionConfig) -> TestResult {
        // TODO: Implement actual audio output test
        if self.config_validator.available_audio_outputs.contains(&device_name.to_string()) {
            TestResult {
                success: true,
    ...
    fn test_midi_output_device(&self, device_name: &str, _config: &MidiSessionConfig) -> TestResult {
        // TODO: Implement actual MIDI test with note-on/note-off
        if self.config_validator.available_midi_devices.contains(&device_name.to_string()) {
            TestResult {
                success: true,
    ...
    fn save_session_config(&self, config: &SessionConfig) -> Result<(), Box<dyn std::error::Error>> {
...
impl ConfigValidator {
    ...
    pub fn validate_session_config(&self, config: &SessionConfig) -> Result<ValidationReport, Box<dyn std::error::Error>> {
    ...
    fn validate_audio_config(&self, config: &AudioSessionConfig, report: &mut ValidationReport) {
        ...
        if !valid_rates.contains(&config.sample_rate) {
            report.add_error("sample_rate", &format!("Unsupported sample rate: {}Hz", config.sample_rate));
        }
        
        // Bit depth validation
        let valid_depths = [16, 24, 32];
        if !valid_depths.contains(&config.bit_depth) {
            report.add_error("bit_depth", &format!("Unsupported bit depth: {}bit", config.bit_depth));
        }
        
        // Buffer size validation
        let valid_buffer_sizes = [128, 256, 512, 1024, 2048, 4096];
        if !valid_buffer_sizes.contains(&config.buffer_size) {
            report.add_error("buffer_size", &format!("Unsupported buffer size: {}", config.buffer_size));
        }
        
        // Device availability validation
        if let Some(ref device) = config.input_device {
            if !self.available_audio_inputs.contains(device) {
                report.add_error("input_device", &format!("Audio input device '{}' not available", device));
            }
        } else {
        ...
        if let Some(ref device) = config.output_device {
            if !self.available_audio_outputs.contains(device) {
                report.add_warning("output_device", &format!("Audio output device '{}' not available", device));
            }
        }
    }
    ...
    fn validate_midi_config(&self, config: &MidiSessionConfig, report: &mut ValidationReport) {
        ...
        if config.channel > 15 {
            report.add_error("midi_channel", &format!("MIDI channel {} out of range (0-15)", config.channel));
        }
        
        // Device availability validation
        if let Some(ref device) = config.output_device {
            if !self.available_midi_devices.contains(device) {
                report.add_error("midi_device", &format!("MIDI device '{}' not available", device));
            }
        } else {
        ...
        if config.program_change_delay_ms > 5000 {
            report.add_warning("program_change_delay", "Program change delay is very high (>5s)");
        }
    }
    
    fn validate_recording_config(&self, config: &RecordingSessionConfig, report: &mut ValidationReport) {
        // Duration validation
        ...
        if config.detection_threshold_db > -6.0 || config.detection_threshold_db < -60.0 {
            report.add_error("detection_threshold", "Detection threshold must be between -60dB and -6dB");
        }
    }
    ...
    fn validate_config_compatibility(&self, config: &SessionConfig, report: &mut ValidationReport) {
        ...
        if config.audio.sample_rate >= 96000 && config.audio.buffer_size < 512 {
            report.add_warning("compatibility", "High sample rate with small buffer size may cause audio glitches");
        }
        ...
        if config.recording.note_duration_ms > 10000 && config.recording.auto_detect_silence {
            report.add_warning("compatibility", "Long note duration with auto-detection may cause premature cutoff");
        }
    }
}
