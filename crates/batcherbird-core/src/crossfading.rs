...
impl EqualPowerCrossfader {
    ...
    pub fn create_loop_crossfade(
        ...
        if loop_start.len() < self.config.length_samples
            || loop_end.len() < self.config.length_samples
        {
            return Err(BatcherbirdError::Audio(
                "Insufficient audio data for crossfade".to_string(),
            ));
        }
        ...
        let phase_offset = if self.config.phase_alignment {
            self.phase_aligner
                .find_optimal_alignment(fade_out_region, fade_in_region)?
        } else {
            0
        };
        ...
        let quality =
            self.assess_crossfade_quality(&crossfaded, fade_out_region, fade_in_region)?;
    ...
    fn assess_crossfade_quality(
        ...
        let spectral_continuity =
            self.calculate_spectral_continuity(crossfaded, original_out, original_in);

        // Overall quality (weighted combination)
        let overall_quality =
            (phase_coherence * 0.4) + (spectral_continuity * 0.4) + ((1.0 - boundary_error) * 0.2);
    ...
    fn calculate_boundary_error(
        ...
        for i in 0..boundary_samples {
            if original_out.len() >= self.config.length_samples + i {
                let diff = crossfaded[i]
                    - original_out[original_out.len() - self.config.length_samples + i];
    ...
    fn calculate_spectral_continuity(
        &self,
        crossfaded: &[f32],
        original_out: &[f32],
        original_in: &[f32],
        ...
        let crossfade_energy =
            crossfaded.iter().map(|&x| x * x).sum::<f32>() / crossfaded.len() as f32;

        let out_region_energy = original_out[original_out.len() - self.config.length_samples..]
            .iter()
            .map(|&x| x * x)
            .sum::<f32>()
            / self.config.length_samples as f32;
...
mod tests {
    ...
    fn test_phase_alignment() {
        ...
        let alignment = aligner
            .find_optimal_alignment(&base_signal, &offset_signal)
            .unwrap();
    ...
    fn test_equal_power_crossfade() {
        ...
        let result = crossfader
            .create_loop_crossfade(&loop_start, &loop_end)
            .unwrap();
    ...
    fn test_crossfade_quality_assessment() {
        ...
        let quality = crossfader
            .assess_crossfade_quality(&crossfaded, &original_out, &original_in)
            .unwrap();
    ...
    fn test_spectral_smoother() {
        ...
        let original_variance = signal[mid_start..mid_end]
            .windows(2)
            .map(|w| (w[1] - w[0]).abs())
            .sum::<f32>();

        let smoothed_variance = smoothed[mid_start..mid_end]
            .windows(2)
            .map(|w| (w[1] - w[0]).abs())
            .sum::<f32>();
