...
impl LoudnessProcessor {
    ...
    pub fn process(&mut self, samples: &[f32]) -> f32 {
        ...
        let mean_square: f32 =
            self.integration_buffer.iter().sum::<f32>() / self.integration_buffer.len() as f32;
...
impl GainStagingAssistant {
    ...
    pub fn analyze_level(
        &mut self,
        professional_readings: &ProfessionalMeterReadings,
    ) -> GainStagingAnalysis {
        ...
        let recommendation = self
            ...
            .recommend_adjustment(reference_level, trend.clone());
        ...
        let is_optimal =
            reference_level >= self.optimal_range.0 && reference_level <= self.optimal_range.1;
    ...
    pub fn get_level_statistics(&self) -> LevelStatistics {
        ...
        let variance = levels.iter().map(|&x| (x - avg).powi(2)).sum::<f32>() / levels.len() as f32;
...
impl ProfessionalMeterEngine {
    ...
    pub fn process_samples(&mut self, samples: &[f32]) -> ProfessionalMeterReadings {
        ...
        // Convert to dB
        let vu_db = if vu_level > 0.0 {
            20.0 * vu_level.log10()
        } else {
            -60.0
        };
        let ppm_db = if ppm_level > 0.0 {
            20.0 * ppm_level.log10()
        } else {
            -60.0
        };
        ...
        let peak_hold_db = if peak_hold > 0.0 {
            20.0 * peak_hold.log10()
        } else {
            -60.0
        };
...
mod tests {
    ...
    fn test_professional_meter_engine() {
        ...
        assert!(
            final_readings.peak_db > -60.0,
            "Peak should be above -60dB after processing samples"
        );
        assert!(
            final_readings.lufs > -70.0,
            "LUFS should be above -70 after processing samples"
        );
    }
