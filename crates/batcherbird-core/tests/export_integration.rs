...
fn test_sfz_export() {
    ...
    let sfz_file = files.iter().find(|f| f.extension().is_some_and(|ext| ext == "sfz")).unwrap();
...
fn test_decent_sampler_export() {
    ...
    let dspreset_file = files.iter().find(|f| f.extension().is_some_and(|ext| ext == "dspreset")).unwrap();
